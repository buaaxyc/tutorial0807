...
public class MyContextMorphTarget : MonoBehaviour
{
    // PUBLIC
    ...
    public int[] visemeToBlendTargets = Enumerable.Range(11, 15).ToArray();
    ...
    public bool enableVisemeTestKeys = true;
    ...
    [Tooltip("Enable using the test keys defined below to manually trigger each expression.")]
    public bool enableExpressionTestKeys = true;
    //[Tooltip("Test key used to manually trigger laughter and visualise the results")]
    public KeyCode smileKey = KeyCode.Alpha4;
    public KeyCode surprisedKey = KeyCode.Alpha5;
    public KeyCode angryKey = KeyCode.Alpha6;
    ...
    public float expressionThreshold = 0.5f;
    ...
    void SetLaughterToMorphTarget(OVRLipSync.Frame frame)
    {
        if (laughterBlendTarget != -1)
        {
            ...
            laughterScore = laughterScore < expressionThreshold ? 0.0f : laughterScore - expressionThreshold;
            laughterScore = Mathf.Min(laughterScore * expressionMultiplier, 1.0f);
            laughterScore *= 1.0f / expressionThreshold;
    ...
    void CheckVisemeKey(KeyCode key, int viseme, int amount)
    {
        if (Input.GetKeyDown(key))
        {
            ...
            lipsyncContext.SetVisemeBlend(viseme, amount);
        }
        if (Input.GetKeyUp(key))
        {
            //lipsyncContext.SetVisemeBlend(visemeToBlendTargets[viseme], 0);
            lipsyncContext.SetVisemeBlend(viseme, 0);
        }
    }
    ...
    void CheckLaughterKey()
    {
        ...
        {
            laughterBlendTarget = 8;
            lipsyncContext.SetLaughterBlend(100);
        }
        if (Input.GetKeyUp(smileKey))
        {
            laughterBlendTarget = 8;
        ...
        {
            laughterBlendTarget = 9;
            lipsyncContext.SetLaughterBlend(100);
        }
        if (Input.GetKeyUp(surprisedKey))
        {
            laughterBlendTarget = 9;
            lipsyncContext.SetLaughterBlend(0);
        }
        ...
        {
            laughterBlendTarget = 10;
            lipsyncContext.SetLaughterBlend(100);
        }
        if (Input.GetKeyUp(angryKey))
        {
            laughterBlendTarget = 10;
            lipsyncContext.SetLaughterBlend(0);
        }
